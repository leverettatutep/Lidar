...
% orig  = [10.1 10.1 10];         % ray's origin
% dir   = [0.5 *n n 0];         % ray's direction
% dir   = [0 0 -1];         % ray's direction
function Hits = FindTheHit(vertices, faces, laser)%, xyz, graph)
    vert1 = vertices(faces(:,1),:);
    vert2 = vertices(faces(:,2),:);
    vert3 = vertices(faces(:,3),:);

    [intersect, AllDist, AllU, AllV, AllCoor] = TriangleRayIntersection( ...
        laser(1:3,4), laser(1:3,3), vert1, vert2, vert3);
    numberHits = sum(intersect);
    if numberHits > 0
        ...
        for i=1:size(intersect)
            if intersect(i)
                baryCenterCoord(hit,:) = [AllU(i) AllV(i)];
                xyzOfHits(hit,:) = AllCoor(i,:);
                distToHits(hit) = AllDist(i);
                hit = hit + 1;
            end
        end
        Hits = distToHits;
...
%LJE I cannot get the graph to work 
%     %%
